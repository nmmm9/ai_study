"""
graph.py - Multi-Agent Debate 패턴

[에이전트 구성]
collector  → 개선된 트렌드 점수로 GitHub 데이터 수집
agent_ai   → AI/ML 전문가 분석
agent_web  → 웹/앱 전문가 분석
agent_sec  → 보안 전문가 분석
supervisor → 3개 분석 종합
critic     → 종합 결과에 반론 제기
judge      → Supervisor vs Critic 최종 결정
report     → 저장

[핵심 개념]
10주차: 에이전트 1개 + Self-Correction
11주차: 전문가 에이전트 3개 + Supervisor + Critic + Judge
"""
