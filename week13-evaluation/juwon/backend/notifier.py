...
def send_keyword_alert(keywords: list[str], language: str = "전체") -> bool:
    gmail_user     = os.getenv("GMAIL_USER", "").strip()
    gmail_password = os.getenv("GMAIL_APP_PASSWORD", "").strip()
    if not gmail_user or not gmail_password:
        return False
    kw_list = ", ".join(f"<strong>{k}</strong>" for k in keywords)
    subject  = f"[GitHub 트렌드 알림] 구독 키워드 감지 ({datetime.now().strftime('%Y-%m-%d')})"
    body_html = f"""
    <html><body style="font-family:sans-serif;max-width:700px;margin:auto">
    <h1 style="color:#0d1117;border-bottom:2px solid #f78166;padding-bottom:8px">🔔 키워드 알림</h1>
    <p>오늘 GitHub 트렌딩에서 구독 중인 키워드가 감지되었습니다.</p>
    <p>언어 필터: <strong>{language}</strong></p>
    <p>감지된 키워드: {kw_list}</p>
    <hr><p style="color:#888;font-size:12px">GitHub Tech Trend Analyzer · Week 13</p>
    </body></html>"""
    msg = MIMEMultipart("alternative")
    msg["Subject"] = subject
    msg["From"]    = gmail_user
