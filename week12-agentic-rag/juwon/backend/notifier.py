"""
notifier.py - 분석 결과 외부 발송 (week11 기반 + 키워드 알림 추가)
"""
import base64
import os
import smtplib
...
def send_keyword_alert(matched: list[dict], language: str = "전체") -> bool:
    """키워드 구독 매칭 알림 메일"""
    gmail_user     = os.getenv("GMAIL_USER", "").strip()
    gmail_password = os.getenv("GMAIL_APP_PASSWORD", "").strip()

    if not gmail_user or not gmail_password or not matched:
        return False

    items_html = "".join(
        f"<li><strong>{m['keyword']}</strong> → "
        f"<a href='{m['repo'].get('url','')}'>{m['repo']['name']}</a> "
        f"(⭐{m['repo'].get('stars', 0):,}): {m['repo'].get('description','')[:80]}</li>"
        for m in matched
    )
    ...
    <p style="color:#888">{datetime.now().strftime('%Y-%m-%d')} | 언어: {language}</p>
    <h2>매칭된 레포</h2>
    <ul>{items_html}</ul>
    <hr>
    <p style="color:#888;font-size:12px">GitHub Tech Trend Analyzer · Week 12 Agentic RAG</p>
    </body></html>
    """

    subject = f"[GitHub 트렌드] 구독 키워드 {len(matched)}건 매칭 ({datetime.now().strftime('%Y-%m-%d')})"
    msg = MIMEMultipart("alternative")
    msg["Subject"] = subject
    msg["From"]    = gmail_user
    msg["To"]      = gmail_user
...
def upload_github_readme(content: str, language: str = "전체", period: str = "weekly") -> bool:
    ...
    if not token or not target_repo:
        print("[notify] 오류: .env에 GITHUB_TOKEN / GITHUB_TREND_REPO 없음")
        return False

    date       = datetime.now().strftime("%Y-%m-%d %H:%M")
    content_md = f"# 📊 GitHub 트렌드 분석\n\n> {date} | 언어: **{language}** | 기간: **{period}**\n\n## ⚖️ Judge 최종 결정\n\n{content}\n\n---\n*Week 12 Agentic RAG*\n"
    ...
    payload: dict = {"message": f"trend report {date}", "content": encoded}
    if sha:
        payload["sha"] = sha
