"""
github_tools.py - GitHub Trending 페이지 스크래핑 (week11과 동일)
"""
import math
import re
...
def calculate_trend_score(stars_this_period: int, total_stars: int, forks: int) -> float:
    velocity_score = min(stars_this_period / 10, 60)
    size_score     = min(math.log1p(total_stars) / 12 * 20, 20)
    fork_score     = min(math.log1p(forks) / 10 * 20, 20)
...
def get_trending_repos(language: str = "", period: str = "weekly", limit: int = 25) -> list:
    since    = {"daily": "daily", "weekly": "weekly", "monthly": "monthly"}.get(period, "weekly")
    ...
    try:
        ...
        for article in articles[:limit]:
            ...
            period_text = period_tag.get_text(strip=True) if period_tag else "0"
            stars_now   = _parse_number(re.sub(r"[^0-9,k]", "", period_text.split()[0]))
            ...
            repos.append({
                ...
                "topics":            [],
                "stars_this_period": stars_now,
                "trend_score":       calculate_trend_score(stars_now, total_stars, forks),
            })
