...
export default function Home() {
  const [mainTab,  setMainTab]  = useState<"analyze" | "evaluate">("analyze");
  ...
  const handleEmail = async () => {
    ...
    try {
      const res  = await fetch(`${API}/api/notify/email`, {
  ...
  const handleGithub = async () => {
    ...
    try {
      const res  = await fetch(`${API}/api/notify/github`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ judge_decision: report.judge_decision ?? "", language: report.language ?? "전체", period: report.period ?? "weekly" }),
      });
      const data = await res.json();
    ...
    <main className="min-h-screen bg-[#0d1117] text-white">
      <div className="border-b border-[#21262d] px-8 py-5">
        ...
        <p className="text-sm text-[#8b949e] mt-1">
          Multi-Agent Debate × Agentic RAG — 과거 분석을 참조하는 지능형 트렌드 분석
        </p>
        {/* 메인 탭 */}
        <div className="flex gap-2 mt-4">
          {([["analyze", "📊 트렌드 분석"], ["evaluate", "🧪 RAG 평가"]] as const).map(([key, label]) => (
        ...
        {mainTab === "analyze" && (
          <>
            ...
            <div className="w-56 shrink-0 space-y-4">
              ...
              <SchedulePanel />
              <KeywordPanel matches={report?.keyword_matches} />
            </div>
            ...
            <div className="flex-1 space-y-6">
              ...
              {report && (
                <>
                  ...
                  <div>
                    <h2 className="text-lg font-bold mb-3 text-[#c9d1d9]">🤖 전문가 에이전트 분석 (RAG 강화)</h2>
                    <div className="grid grid-cols-3 gap-4">
