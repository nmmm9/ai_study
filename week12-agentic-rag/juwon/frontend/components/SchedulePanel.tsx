...
export default function SchedulePanel() {
  ...
  useEffect(() => {
    fetch(`${API}/api/schedule`)
      .then((r) => r.json())
      .then((d) => { setEnabled(d.enabled); setHour(d.hour); setPeriod(d.period); setNextRun(d.next_run); })
      .catch(() => {});
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setNextRun(data.next_run); setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch { alert("저장 실패: 백엔드 서버를 확인해주세요."); }
        ...
        <>
          ...
          <div>
            ...
            <select value={period} onChange={(e) => setPeriod(e.target.value)}
              className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-3 py-1.5 text-sm text-white">
              {Object.entries(PERIODS).map(([label, val]) => <option key={val} value={val}>{label}</option>)}
            </select>
          </div>
          {nextRun && <p className="text-xs text-[#484f58]">다음 실행: {new Date(nextRun).toLocaleString("ko-KR")}</p>}
        </>
      )}
