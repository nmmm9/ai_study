...
type Props = { title: string; icon: string; content: string; color: string };
  ...
  return (
    <div className={`bg-[#161b22] border ${c.border} rounded-xl flex flex-col`}>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[#21262d]">
        ...
        <span className="text-xs text-[#484f58] ml-1">RAG 참조</span>
      </div>
      <div className="px-4 py-3 overflow-y-auto max-h-72">
        {content ? <Markdown content={content} /> : <p className="text-xs text-[#484f58]">분석 중...</p>}
      </div>
    </div>
  );
}
