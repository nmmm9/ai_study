...
async def daiso_pickup_stock(pd_no: str, str_cd: str) -> dict:
    # 다이소 API는 CSRF 토큰/세션 인증을 요구하기 시작. warm-up 후에도 403 가능.
    async with httpx.AsyncClient(timeout=12, follow_redirects=True) as client:
        ...
        if resp.status_code == 403:
            return {
                "pd_no": pd_no,
                "str_cd": str_cd,
                "error": "다이소 API 인증 거부 (403)",
                "reason": "Daisomall 픽업 재고 API 가 CSRF/세션 검증을 강화함",
                "alternative": f"https://www.daisomall.co.kr/onl/onlPd/onlPdInfo.do?pdNo={pd_no} 에서 직접 확인",
            }
        if resp.status_code != 200:
            return {
                "pd_no": pd_no,
                "str_cd": str_cd,
                "error": f"재고 조회 실패 (status {resp.status_code})",
                "body": resp.text[:200],
            }
        ...
        try:
            ...
            return {"error": f"재고 파싱 실패: {e}", "body": resp.text[:200]}
