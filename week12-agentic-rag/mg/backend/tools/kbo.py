...
async def kbo_results(date: str = None) -> dict:
    ...
    async with httpx.AsyncClient(timeout=12, follow_redirects=True) as client:
        # 1) 메인 페이지 한 번 호출해서 cookie/session 받음
        await client.get(
        ...
        if resp.status_code != 200:
            return {"date": date, "error": f"KBO 조회 실패 (status {resp.status_code})"}

        raw = (resp.text or "").strip()
        if not raw:
            return {"date": date, "error": "KBO 빈 응답 — 시즌 외 기간이거나 API 변경 가능", "games": []}

        # ASP.NET 응답이 가끔 <string xmlns="...">JSON</string> 으로 래핑됨
        m = re.match(r'^\s*<string[^>]*>(.+?)</string>\s*$', raw, re.DOTALL)
        if m:
            raw = m.group(1)
        ...
        try:
            ...
            return {
                "date": date,
                "error": f"파싱 실패: {str(e)}",
                "raw_snippet": raw[:300] if raw else "(empty)",
                "hint": "KBO 정규 시즌은 보통 3월~10월. 2026-05-24는 정규 시즌이지만 API 응답이 빈 경우 koreabaseball.com 직접 확인 권장.",
            }
