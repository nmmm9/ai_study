"""
2주차: Chunking - Streamlit 웹 앱
취업공고 문서를 청킹하고 GPT로 질의응답하는 웹 인터페이스
"""
...
# chunking_chat.py에서 DocumentLoader, TextChunker 재사용
...
def find_relevant_chunks(query: str) -> list[str]:
    """키워드 매칭으로 관련 청크 상위 TOP_K개 반환"""
    chunks = st.session_state.chunks
    if not chunks:
        return []
    keywords = query.split()
    scored = sorted(
        ((sum(c.lower().count(kw.lower()) for kw in keywords), c) for c in chunks),
        reverse=True,
    )
...
def main():
    ...
    st.title("💼 취업공고 챗봇")
    st.caption("2주차 — PDF/Markdown 로드 + 텍스트 청킹 + GPT 질의응답")
    ...
    if prompt := st.chat_input("취업공고에 대해 궁금한 점을 물어보세요"):
        ...
        with st.chat_message("assistant"):
            if relevant:
                ...
                st.caption("⚠️ 관련 청크를 찾지 못했습니다 (키워드 매칭 실패)")
