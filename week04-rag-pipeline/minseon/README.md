...
## WHY (의사결정 기록)
1. **Q**: 왜 이 방식을 선택했는가?
   **A**:  왜 이 방식(RAG)을 선택했는가? 
   GPT 같은 LLM은 학습 데이터에 없는 최신 정보나 특정 문서(청년 정책 자료)를 모른다. 그냥 GPT에게 물어보면 "모른다"거나 잘못된 답변을 합니다. RAG는 문서를 미리 벡터로 저장해두고, 질문이 들어오면 관련 내용을 꺼내서 GPT에게 "이거 참고해서 답해"라고 주입합니다. LLM의 지식 한계를 내가 정리한 문서로 보완하는 방식이라 선택했다.


2. **Q**: 다르게 구현한다면 어떻게 했을까?
   **A**: 프론트엔드: CDN React 대신 Vite + React로 구성하면 컴포넌트 파일 분리, 핫 리로드 등 개발의 편의성을 좋게 만들었을거 같다> 
...
linkify 호출이 남아있어서 에러 발생 (함수가 없음)
marked.js 최신 버전에서 renderer.link 함수 시그니처가 변경됨

## 회고
- 이번 주 배운 점: RAG파이프 라인의 흐름과 필요한 기술에 대해 알게되었다
- 다음 주 준비할 것: 질 좋은 데이터
